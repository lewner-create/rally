  ...
  return (
    <>
      ...
      <button
        ...
        style={{ background: '#fff', color: '#333', border: '1px solid #ddd', opacity: googlePending ? 0.7 : 1 }}
      >
        {googlePending ? <span>Redirecting...</span> : <><GoogleIcon /><span>Continue with Google</span></>}
      </button>
      ...
      <form action={action} className="space-y-4">
        ...
        {state?.error && (
          <div className="rounded-lg px-3 py-2.5 text-sm" style={{ background: '#FAECE7', color: '#712B13' }}>
            {state.error}
          </div>
        )}
      ...
      <p className="text-sm text-muted-foreground text-center mt-5">
        No account?{' '}
        <Link href="/signup" className="font-medium" style={{ color: '#7F77DD' }}>Sign up</Link>
      </p>
    </>
  )
}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div style={{ color: '#555', fontSize: 14 }}>Loading...</div>}>
      <LoginForm />
    </Suspense>
  )
}
...
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18">
      ...
      <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.258c-.806.54-1.837.86
[System.IO.File]::WriteAllText(
  "D:\Rally\rally\apps\web\src\app\(auth)\login\page.tsx",
  @'
'use client'
