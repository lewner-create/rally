...
export default function RequestAccessPage() {
  ...
  return (
    <div style={{
      ...
      {/* Card */}
      <div style={{ width: '100%', maxWidth: '360px', background: '#fff', borderRadius: 20, padding: '32px', boxShadow: '0 32px 80px rgba(0,0,0,0.35), 0 4px 16px rgba(0,0,0,0.2)', position: 'relative' }}>
        {done ? (
          /* ── Success state ── */
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <Link href="/" style={{ fontSize: 13, color: accent, textDecoration: 'none', fontWeight: 500 }}>
              ← Back to home
            </Link>
          </div>
        ) : (
          /* ── Request form ── */
          <>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: '#111', margin: '0 0 6px', letterSpacing: '-0.02em' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {error && (
                <div style={{ background: '#FAECE7', color: '#712B13', padding: '10px 14px', borderRadius: 10, fontSize: 13 }}>
                  {error}
                </div>
              )}
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 14, fontWeight: 700, cursor: (name.trim() && email.trim()) ? 'pointer' : 'default',
            ...
            <p style={{ textAlign: 'center', fontSize: 12, color: '#aaa', marginTop: 20 }}>
              Already have access?{' '}
              <Link href="/login" style={{ color: accent, textDecoration: 'none', fontWeight: 500 }}>
                Sign in
              </Link>
            </p>
          </>
