...
export async function generateInviteToken(
  ...
  const { data: tokenRow, error } = await supabase
    .from('invite_tokens')
    .insert({ inviter_id: user.id, label: label?.trim() || null })
    .select('token')
    .single()
...
// Uses service role so unauthenticated visitors can view invite details.
...
// ─── Consume token on signup (called from welcome page server action) ──────
...
export async function consumeInviteToken(params: {
  ...
  // 3. Create an approved access_request so the auth gate passes
  await admin
