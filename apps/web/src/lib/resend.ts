...
export async function sendInviteEmail(opts: {
  ...
  await getResend().emails.send({
    ...
    html: '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>' +
      ...
      '<p style="margin:0 0 28px;font-size:14px;color:#aaa;line-height:1.6"><strong style="color:#fff">' + opts.inviterName + '</strong> invited you to join this event on Volta.</p>' +
      ...
      '</div>' +
      '<p style="margin:20px 0 0;font-size:11px;color:#444;text-align:center">You were invited by ' + opts.inviterName + ' via Volta</p>' +
      '</div></body></html>',
  })
}
